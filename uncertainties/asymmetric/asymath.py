...
# uncertainties by returning a AsyAffineScalarFunc object.

# Some functions from the math module cannot be adapted in a standard
# way so to work with AsyAffineScalarFunc objects (either as their result
# or as their arguments):

# (1) Some functions return a result of a type whose value and
# variations (uncertainties) cannot be represented by AsyAffineScalarFunc
...
def wrap_locally_cst_func(func):
    ...
    after converting any AsyAffineScalarFunc object to its nominal value.
...
def modf(x):
    ...
    aff_func = to_asyaffine_scalar(x)  # Uniform treatment of all numbers
    ...
    if aff_func._linear_part:  # If not a constant
        ...
        return (AsyAffineScalarFunc(frac_part, aff_func._linear_part), int_part)
    else:
        # This function was not called with an AsyAffineScalarFunc
...
def ldexp(x, i):
    ...
    aff_func = to_asyaffine_scalar(x)  # y must be an integer, for math.ldexp
...
def frexp(x):
    ...
    aff_func = to_asyaffine_scalar(x)
