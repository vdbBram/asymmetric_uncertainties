...
__all__ = [
    ...
    'asyfloat',  # Main function: returns a number with uncertainty
    'ufloat_fromstr',  # Important function: returns a number with uncertainty

    # Uniform access to nominal values and upper and lower standard deviations:
    'nominal_value',
    'lower_std_dev',
    'upper_std_dev',
...
try:
    ...
    def correlated_values(nom_values, covariance_mat, tags=None):
        """
        Return numbers with uncertainties (AsyAffineScalarFunc objects)
        ...
        created, independent variables (AsyVariable objects).
...
def to_asyaffine_scalar(x):
    """
    Transforms x into a constant affine scalar function
    (AsyAffineScalarFunc), unless it is already an AsyAffineScalarFunc (in
    which case x is returned unchanged).

    Raises an exception unless x belongs to some specific classes of
    objects that are known not to depend on AsyAffineScalarFunc objects
    (which then cannot be considered as constants).
    """

    if isinstance(x, AsyAffineScalarFunc):
        return x

    if isinstance(x, CONSTANT_TYPES):
        # No variable => no derivative:
        return AsyAffineScalarFunc(x, LinearCombination({}))
...
def wrap(f, derivatives_args=[], derivatives_kwargs={}):
    ...
    uncertainties (ASYFloat objects); the wrapped function returns the
    ...
    def f_with_affine_output(*args, **kwargs):
        ...
        # The arguments that contain an uncertainty (AsyAffineScalarFunc
        ...
        pos_w_uncert = [index for (index, value) in enumerate(args)
                        if isinstance(value, AsyAffineScalarFunc)]
        ...
        # AsyAffineScalarFunc.__mul__ operator, obtained through wrap(),
        ...
        # is an AsyAffineScalarFunc (for all AsyAffineScalarFunc found as
        # argument of f):
        linear_part = []
...
def force_aff_func_args(func):
    ...
    The constructed operator returns func(x, to_asyaffine_scalar(y)) if y
    can be upcast with to_asyaffine_scalar(); otherwise, it returns
    NotImplemented.
    ...
    def op_on_upcast_args(x, y):
        """
        Return %s(self, to_asyaffine_scalar(y)) if y can be upcast
        through to_asyaffine_scalar.  Otherwise returns NotImplemented.
        """ % func.__name__

        try:
            y_with_uncert = to_asyaffine_scalar(y)
        except NotUpcast:
...
def PDG_precision(lower_std_dev, upper_std_dev):
    '''
    ...
    exponent = min([first_digit(lower_std_dev),first_digit(upper_std_dev)])
    ...
    digits = int(lower_std_dev/10.**exponent*factor)  # int rounds towards zero

    # Rules:
    if digits <= 354:
        return (2, lower_std_dev,upper_std_dev)
    elif digits <= 949:
        return (1, lower_std_dev,upper_std_dev)
    else:
...
def format_num(nom_val_main, lower_error_main, upper_error_main, common_exp,
    ...
    # over common_exp):
    special_error = not lower_error_main or isinfinite(lower_error_main) or not upper_error_main or isinfinite(upper_error_main)
    ...
    if 'S' in options:  # Shorthand notation:
        ...
        if lower_error_main == 0:
            # The error is exactly zero
            lower_uncert_str = '0'
        elif isnan(lower_error_main):
            lower_uncert_str = robust_format(lower_error_main, main_pres_type)
            if 'L' in options:
                lower_uncert_str = r'\mathrm{%s}' % lower_uncert_str
        elif isinf(lower_error_main):
            if 'L' in options:
                lower_uncert_str = r'\infty'
            else:
                lower_uncert_str = robust_format(lower_error_main, main_pres_type)
        else:  #  Error with a meaningful first digit (not 0, and real number)

            lower_uncert = round(lower_error_main, prec)
            ...
            # notation:
            if first_digit(lower_uncert) >= 0 and prec > 0:
                # This case includes a zero rounded error with digits
                # after the decimal point:
                lower_uncert_str = '%.*f' % (prec, lower_uncert)
                ...
                if lower_uncert:
                    ...
                    lower_uncert_str = '%d' % round(lower_uncert*10.**prec)
                else:
        ...
        if upper_error_main == 0:
            # The error is exactly zero
            upper_uncert_str = '0'
        elif isnan(upper_error_main):
            upper_uncert_str = robust_format(upper_error_main, main_pres_type)
            if 'L' in options:
                upper_uncert_str = r'\mathrm{%s}' % upper_uncert_str
        elif isinf(upper_error_main):
            if 'L' in options:
                upper_uncert_str = r'\infty'
            else:
                upper_uncert_str = robust_format(upper_error_main, main_pres_type)
        else:  #  Error with a meaningful first digit (not 0, and real number)

            upper_uncert = round(upper_error_main, prec)
            ...
            # notation then essentially coincides with the +/-
            # notation:
            if first_digit(upper_uncert) >= 0 and prec > 0:
                # This case includes a zero rounded error with digits
                # after the decimal point:
                upper_uncert_str = '%.*f' % (prec, upper_uncert)
                ...
                if upper_uncert:
                    ...
                    # first be obtained when 567 is wanted (%d prints the
                    # integer part, not the rounded value):
                    upper_uncert_str = '%d' % round(upper_uncert*10.**prec)
                else:
                    # The decimal point indicates a truncated float
                    # (this is easy to do, in this case, since
                    # fmt_prefix_e is ignored):
        ...
        # the width:
        value_end = '(%s)%s%s%s' % (upper_uncert_str, lower_uncert_str, exp_str, percent_str)
        ...
        # zero uncertainty:
        if lower_error_main and upper_error_main:
        ...
        lower_error_str = robust_format(lower_error_main, fmt_prefix_e+fmt_suffix_e) #TODO
        upper_error_str = robust_format(upper_error_main, fmt_prefix_e+fmt_suffix_e)
        ...
        if 'L' in options:
            ...
            if isnan(lower_error_main):
                lower_error_str = r'\mathrm{%s}' % lower_error_str
            elif isinf(lower_error_main):
                lower_error_str = r'\infty'
            if isnan(upper_error_main):
                upper_error_str = r'\mathrm{%s}' % upper_error_str
            elif isinf(upper_error_main):
                upper_error_str = r'\infty'

        if nom_has_exp:
            nom_val_str += exp_str
        if error_has_exp:
            lower_error_str += exp_str
        if error_has_exp:
            upper_error_str += exp_str
        ...
        if any_exp_factored and common_exp is not None:  # Exponent
            value_str = ''.join((
                LEFT_GROUPING,
                nom_val_str, '+', upper_error_str, '/-', lower_error_str,
                RIGHT_GROUPING,
                exp_str, percent_str))
        else:  # No exponent
            value_str = ''.join([nom_val_str, '+', upper_error_str, '/-', lower_error_str])
            if percent_str:
                value_str = ''.join((
...
class AsyAffineScalarFunc(object):
    """
    ...
    def lower_error_components(self):
        """
        ...
        for (variable, derivative) in self.derivatives.items():
            ...
            if variable._lower_std_dev == 0:
                # !!! Shouldn't the errors always be floats, as a
                # convention of this module?
                error_components[variable] = 0
            else:
                error_components[variable] = abs(derivative*variable._lower_std_dev)

        return error_components

    def upper_error_components(self):
        """
        Individual components of the standard deviation of the affine
        function (in absolute value), returned as a dictionary with
        Variable objects as keys. The returned variables are the
        ...
        for (variable, derivative) in self.derivatives.items():
            ...
            # variable obviously leads to no uncertainty in the
            # functions that depend on it.
            if variable._upper_std_dev == 0:
                # !!! Shouldn't the errors always be floats, as a
                # convention of this module?
                error_components[variable] = 0
            else:
                error_components[variable] = abs(derivative*variable._upper_std_dev)

        return error_components

    @property
    def lower_std_dev(self):
        """
        ...
        return CallableStdDev(sqrt(sum(
            delta**2 for delta in self.lower_error_components().values())))

    @property
    def upper_std_dev(self):
        """
        Standard deviation of the affine function.
        ...
        #AffineScalarFunc returned to the user does).
        return CallableStdDev(sqrt(sum(
            delta**2 for delta in self.upper_error_components().values())))
    ...
    def __repr__(self):
        ...
        lower_std_dev = self.lower_std_dev  # Optimization, since std_dev is calculated
        upper_std_dev = self.upper_std_dev  # Optimization, since std_dev is calculated
        ...
        if lower_std_dev:
            lower_std_dev_str = repr(lower_std_dev)
        else:
            lower_std_dev_str = '0'
        if upper_std_dev:
            upper_std_dev_str = repr(upper_std_dev)
        else:
            upper_std_dev_str = '0'

        return "%r+%s/-%s" % (self.nominal_value, upper_std_dev_str, lower_std_dev_str)
    ...
    def __format__(self, format_spec):
        ...
        lower_std_dev = self.lower_std_dev
        upper_std_dev = self.upper_std_dev
        ...
        real_values = [value for value in [abs(nom_val), lower_std_dev, upper_std_dev]
        ...
        if ((
            ...
            and lower_std_dev
            and upper_std_dev
            and not isinfinite(lower_std_dev)
            and not isinfinite(upper_std_dev)):
            ...
            if fmt_prec:
                ...
                (num_signif_d, lower_std_dev, upper_std_dev) = PDG_precision(lower_std_dev, upper_std_dev)

            digits_limit = max([signif_dgt_to_limit(lower_std_dev, num_signif_d),signif_dgt_to_limit(upper_std_dev, num_signif_d)])
        ...
        if use_exp:
            ...
            lower_std_dev_mantissa = lower_std_dev
            upper_std_dev_mantissa = upper_std_dev
        ...
        if signif_limit is not None:  # If signif_limit is pertinent
            ...
            prec = max(-signif_limit,
                       1 if pres_type is None and not lower_std_dev and not upper_std_dev
                       else 0)
        ## print "PREC", prec
        ...
        # Final formatting:
        return format_num(nom_val_mantissa, lower_std_dev_mantissa, upper_std_dev_mantissa, common_exp,
                          match.groupdict(),
                          prec=prec,
    ...
    def upper_std_score(self, value):
        """
        ...
        try:
            ...
            # here:
            return (value - self._nominal_value) / self.lower_std_dev
        except ZeroDivisionError:
            raise ValueError("The standard deviation is zero:"
                             " undefined result")

    def lower_std_score(self, value):
        """
        Return 'value' - nominal value, in units of the standard
        deviation.
        ...
        try:
            # The ._nominal_value is a float: there is no integer division,
            # here:
            return (value - self._nominal_value) / self.upper_std_dev
        except ZeroDivisionError:
            raise ValueError("The standard deviation is zero:"
                             " undefined result")
    ...
    def __deepcopy__(self, memo):
        ...
        """
        return AsyAffineScalarFunc(self._nominal_value,
...
def add_operators_to_AffineScalarFunc():
    ...
    for (op, derivative) in (
        ...
        try:
            setattr(AsyAffineScalarFunc, attribute_name,
    ...
    for coercion_type in ('complex', 'int', 'long', 'float'):
        ...
        setattr(AsyAffineScalarFunc, '__%s__' % coercion_type, raise_error)
...
class AsyVariable(AsyAffineScalarFunc):
    """
    ...
    # To save memory in large arrays:
    __slots__ = {'_lower_std_dev', '_upper_std_dev', 'tag'}

    def __init__(self, nominal_value, lower_std_dev, upper_std_dev, tag=None):
        """
        ...
        super(AsyVariable, self).__init__(value, LinearCombination({self: 1.}))

        self.upper_std_dev = upper_std_dev
        self.lower_std_dev = lower_std_dev
        self.tag = tag

    @property
    def upper_std_dev(self):
        return self._upper_std_dev

    @property
    def lower_std_dev(self):
        return self._lower_std_dev
    ...
    @upper_std_dev.setter
    def upper_std_dev(self, upper_std_dev):
        ...
        if upper_std_dev < 0 and not isinfinite(upper_std_dev):
            raise NegativeStdDev("The standard deviation cannot be negative")

        self._upper_std_dev = CallableStdDev(upper_std_dev)

    # Standard deviations can be modified (this is a feature).
    # AffineScalarFunc objects that depend on the Variable have their
    # std_dev automatically modified (recalculated with the new
    # std_dev of their Variables):
    @lower_std_dev.setter
    def lower_std_dev(self, lower_std_dev):
        ...
        # separately for NaN. But this is not guaranteed, even if it
        # should work on most platforms.)
        if lower_std_dev < 0 and not isinfinite(lower_std_dev):
            raise NegativeStdDev("The standard deviation cannot be negative")

        self._lower_std_dev = CallableStdDev(lower_std_dev)

    # Support for legacy method:
    def set_upper_std_dev(self, value):  # Obsolete
        deprecation('instead of set_std_dev(), please use'
                    ' .std_dev = ...')
        self.upper_std_dev = value

    # Support for legacy method:
    def set_lower_std_dev(self, value):  # Obsolete
        deprecation('instead of set_std_dev(), please use'
                    ' .std_dev = ...')
        self.lower_std_dev = value 

    # The following method is overridden so that we can represent the tag:
    def __repr__(self):

        num_repr  = super(AsyVariable, self).__repr__()
    ...
    def __copy__(self):
        ...
        return AsyVariable(self.nominal_value, self.lower_std_dev, self.upper_std_dev, self.tag)
...
def lower_std_dev(x):
    ...
    if isinstance(x, AsyAffineScalarFunc):
        return x.lower_std_dev
    else:
        return 0.

def lower_covariance_matrix(nums_with_uncert):
    """
    ...
    for (i1, expr1) in enumerate(nums_with_uncert, 1):
        ...
        for expr2 in nums_with_uncert[:i1]:
            ...
            coefs_expr1.append(sum(
                ((derivatives1[var]*derivatives2[var]*var._lower_std_dev**2)
...
def upper_covariance_matrix(nums_with_uncert):
    """
    Return a matrix that contains the covariances between the given
    sequence of numbers with uncertainties (AffineScalarFunc objects).
    The resulting matrix implicitly depends on their ordering in
    ...
    for (i1, expr1) in enumerate(nums_with_uncert, 1):
        ...
        for expr2 in nums_with_uncert[:i1]:
            derivatives2 = expr2.derivatives  # Optimization
            coefs_expr1.append(sum(
                ((derivatives1[var]*derivatives2[var]*var._upper_std_dev**2)
                # var is a variable common to both numbers with
                # uncertainties:
                for var in vars1.intersection(derivatives2)),
                # The result is always a float (sum() with no terms
...
try:
    ...
    def upper_correlation_matrix(nums_with_uncert):
        '''
        Return the correlation matrix of the given sequence of
        numbers with uncertainties, as a NumPy array of floats.
        '''

        cov_mat = numpy.array(upper_covariance_matrix(nums_with_uncert))

        std_devs = numpy.sqrt(cov_mat.diagonal())

        return cov_mat/std_devs/std_devs[numpy.newaxis].T

    __all__.append('upper_correlation_matrix')
...
# supported anymore.
def asyfloat(nominal_value, lower_std = None, upper_std = None, tag = None):
    """
    ...
    try:
        # Standard case:
        return AsyVariable(nominal_value, lower_std, upper_std, tag = tag)
