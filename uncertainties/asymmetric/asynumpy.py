...
__all__ = [
    ...
    # Utilities:
    'nominal_values', 'upper_std_devs', 'lower_std_devs'
...
def upper_std_devs(arr):
    ...
    return unumpy_to_numpy_matrix(to_upper_std_devs(arr))

def lower_std_devs(arr):
    """
    Return the standard deviations of the numbers in NumPy array arr.
    ...
    return unumpy_to_numpy_matrix(to_lower_std_devs(arr))
...
def derivative(u, var):
    ...
    """
    if isinstance(u, uncert_core.AsyAffineScalarFunc):
        try:
...
def wrap_array_func(func):
    ...
    def wrapped_func(arr, *args, **kwargs):
        ...
        for element in arr.flat:
            ...
            if isinstance(element, uncert_core.AsyAffineScalarFunc):
        ...
        for var in variables:
            ...
            shift_var = max(var._lower_std_dev/1e5, var._upper_std_dev/1e5, 1e-8*abs(var._nominal_value))
        ...
        return numpy.vectorize(uncert_core.AsyAffineScalarFunc)(
            func_nominal_value,
...
def asyarray(nominal_values, lower_std_devs=None, upper_std_devs=None):
    """
    ...
    if upper_std_devs is None and lower_std_devs is None:  # Obsolete, single tuple argument call
        deprecation('asyarray() should now be called with two arguments.')
        (nominal_values, lower_std_devs, upper_std_devs) = nominal_values
    ...
    return (numpy.vectorize(
        ...
        lambda v, s, g: uncert_core.AsyVariable(v, s, g), otypes=[object])
        (nominal_values, lower_std_devs, upper_std_devs))
...
def func_with_deriv_to_uncert_func(func_with_derivatives):
    ...
    **kwargs) must return an iterator.  The first element returned by
    ...
    def wrapped_func(array_like, *args, **kwargs):
        ...
        # result:
        result = numpy.vectorize(uncert_core.AsyAffineScalarFunc)(
            func_nominal_value,
            numpy.vectorize(uncert_core.LinearCombination)(derivatives))
...
class matrix(numpy.matrix):
    ...
    # the first ones to have such methods?
    @property
    def upper_std_devs(self):
        return CallableStdDevs(upper_std_devs(self))
    ...
    # the first ones to have such methods?
    @property
    def lower_std_devs(self):
        return CallableStdDevs(lower_std_devs(self))

def asymatrix(nominal_values, lower_std_devs=None, upper_std_devs=None):
    """
    Constructs a matrix that contains numbers with uncertainties.

    The arguments are the same as for asyarray(...): nominal values, and
    standard deviations.
    ...
    if lower_std_devs is None and upper_std_devs is None:  # Obsolete, single tuple argument call
        deprecation('umatrix() should now be called with two arguments.')
        (nominal_values, lower_std_devs, upper_std_devs) = nominal_values

    return asyarray(nominal_values, lower_std_devs, upper_std_devs).view(matrix)
